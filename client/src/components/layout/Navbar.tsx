      ...
      <nav className="border-b bg-white/80 backdrop-blur-md sticky top-0 z-50 safe-top" data-testid="navbar">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 safe-left safe-right">
          <div className="flex justify-between h-14 sm:h-16 items-center">
            <Link href="/" className="flex items-center gap-2 group touch-manipulation" data-testid="link-home-logo">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-t safe-bottom" data-testid="mobile-tab-bar">
        <div className="flex items-center justify-around h-16 safe-left safe-right">
          <Link href="/" data-testid="link-translate-mobile">
            <div className={`flex flex-col items-center gap-1 px-6 py-2 rounded-xl transition-colors touch-manipulation ${location === '/' ? 'text-primary' : 'text-muted-foreground'}`}>
              <Home className="h-5 w-5" />
              <span className="text-xs font-medium">Translate</span>
            </div>
          </Link>
          <Link href="/history" data-testid="link-history-mobile">
            <div className={`flex flex-col items-center gap-1 px-6 py-2 rounded-xl transition-colors touch-manipulation ${location === '/history' ? 'text-primary' : 'text-muted-foreground'}`}>
